...
- [ ] **Stage 1: Read endpoints from DB** — `GET /tasks` & `GET /tasks/{id}` reading from SQLite.
...
3. **Verify Database Initialization**:
   - `tasks.db` file will be created automatically.
   - Initial 3 example tasks are seeded on first launch.
   - Restarting the server multiple times preserves data without duplicating seeds.
